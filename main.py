...
from scripts.data_augmentation import offline_augment_dataset
...
def parse_args():
    ...
    # Data augmentation
    parser.add_argument("--offline_aug_count", type=int, default=5,
                        help="Number of offline augmentations per image")
...
def augment_data(args):
    ...
    logger.info(f"Offline augmenting {args.dataset} dataset...")

    raw_data_dir = args.data_dir + '/raw/train/'
    augmented_data_dir = args.data_dir + '/augmented/train/'
    offline_aug_count = args.offline_aug_count
    ...
    if not os.path.exists(raw_data_dir):
        ...
        print(f"   Number of offline augmentations per image for standard images: {offline_aug_count}")

    # Ensure the raw data directory exists before running
    if os.path.exists(raw_data_dir):
        if os.path.exists(augmented_data_dir):
            print("✅ Skipping offline augmentation process as offline augmentation already performed.")
        else:
            print("🚀 Starting offline augmentation...")
            offline_augment_dataset(
                input_dir=raw_data_dir,
                output_dir=augmented_data_dir,
                offline_aug_count=offline_aug_count
            )
            print("\n🎉 Offline data augmentation completed successfully!")
    else:
        print("Skipping offline augmentation process due to missing raw data directory.")
...
def train(args, model: nn.Module):
    ...
    val_dir = args.data_dir + "/raw/val"

    train_loader, val_loader = load_combined_data(augmented_train_dir, raw_train_dir, val_dir, args.batch_size)

    # Initialize EMA
    ema = ModelEMA(model, decay=float(os.environ.get("EMA_DECAY", 0.999)))
    ...
    for epoch in range(args.num_epochs):
        ...
        train_loss, train_acc = train_epoch(
            model, train_loader, criterion, optimizer, args.device, scaler=scaler, ema=ema
        )
        ...
        # Update learning rate not based on validation loss
        scheduler.step()
    ...
    # Restore scaler state if exists
    ...
    if ema is not None:
        ...
        ema.restore(model)
        print("💾 Final model saved with EMA weights")
    else:
        torch.save(model.state_dict(), args.output_dir + "/models/final_model.pth")
